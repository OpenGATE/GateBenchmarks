...
def analyse_all_folders(output_folders):
    r_proton = analyse_one_particle(output_folders, 'proton')
    r_carbon = analyse_one_particle(output_folders, 'carbon')
    print(f'Last test return is: proton {r_proton} and carbon {r_carbon}')
    return r_proton & r_carbon


def analyse_one_particle(output_folders, particle_name):
