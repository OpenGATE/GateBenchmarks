...
def analyze(folder, type):
    if type == "energyWinner":
        file= os.path.join(folder, "test_energyWinner.root")
    elif type == "centroid":
        file= os.path.join(folder, "test_centroid.root")
            ...
            for j in range(len(tmp_blockID)) :
                for k in range(n_unique_blockID) :
                    if ( tmp_blockID[j] ==  unique_blockID[k]) :
                        if type == "energyWinner" and (tmp_edep_hit[j] >= maxEdep[k]) : #find max energy and position of energy winner
...
def analyse_one_folder(folder):
    ...
    r_ew = analyze(folder, "energyWinner") 

    # analyze centroid
    r_c = analyze(folder, "centroid")
