...
def compare_with_previous_version(file_new, file_old):
    ...
    hitTree_new = uproot.open(file_new)['Hits_crystal'].arrays(branches_new,library="numpy")
    ...
    hitTree2_new = uproot.open(file_new)['Hits_crystal2'].arrays(branches_new,library="numpy")
    edep2_new=hitTree2_new["edep"]
    posX2_new=hitTree2_new["posX"]
    posY2_new=hitTree2_new["posY"]
    posZ2_new=hitTree2_new["posZ"]
    time2_new=hitTree2_new["time"]

    edep_new_merged=np.concatenate([edep_new,edep2_new])
    posX_new_merged=np.concatenate([posX_new,posX2_new])
    posY_new_merged=np.concatenate([posY_new,posY2_new])
    posZ_new_merged=np.concatenate([posZ_new,posZ2_new])
    time_new_merged=np.concatenate([time_new,time2_new])
    ...
    edep_new_merged.sort() 
    posX_new_merged.sort()
    posY_new_merged.sort()
    posZ_new_merged.sort()
    time_new_merged.sort() 
        

    acceptance = 0.00001   
    for i in range(len(edep_old)) : 
        if (math.fabs(edep_old[i]-edep_new_merged[i])>acceptance):
            print(CRED+"Energy test failed"+CEND, edep_old[i], " vs. ",  edep_new_merged[i])
            exit()
            returnBool = False
        if (math.fabs(time_old[i]-time_new_merged[i])>acceptance):
            print(CRED+"Min time test failed"+CEND, time_old[i], " vs. ", time_new_merged[i])
            exit()
            returnBool = False
        if (math.fabs(posX_old[i]-posX_new_merged[i])>acceptance):
            print(CRED+"PosX test failed"+CEND, posX_old[i], " vs. ", posX_new_merged[i])
            exit()
            returnBool = False
        if (math.fabs(posY_old[i]-posY_new_merged[i])>acceptance):
            print(CRED+"PosY test failed"+CEND, posY_old[i], " vs. ", posY_new_merged[i])
            exit()
            returnBool = False
        if (math.fabs(posZ_old[i]-posZ_new_merged[i])>acceptance):
            print(CRED+"PosZ time test failed"+CEND, posZ_old[i], " vs. ", posZ_new_merged[i])
            exit()
            returnBool = False
